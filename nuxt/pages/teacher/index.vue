      ...
      <section class="c-sort-box unBr">
        <div>
          ...
          <article class="i-teacher-list" v-if="map.total>0">
            <ul class="of">
              <li v-for="teacher in map.rows" :key="teacher.id"> 
                <section class="i-teach-wrap">
                  <div class="i-teach-pic">
                    <a :href='["/teacher/"+teacher.id]' :title="teacher.name" target="_blank">
                      <!-- :src = "teacher.avatar" 上线改 -->
                      <img src="~/assets/photo/teacher/1442297885942.jpg" :alt="teacher.name">
                    </a>
                  </div>
...
export default {
  ...
  methods: {
    getList(){
      getTeacherList(1,8).then(response => {
        this.map = response.data.data;
      })
    },
    gotoPage(page){
      getTeacherList(page,8).then(response => {
        this.map = response.data.data;
      })
    }
  }
