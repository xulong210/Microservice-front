          ...
          <dl>
            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                ...
                <li :class="{active:oneIndex==index}" v-for="(item,index) in subjectNestedList" :key="index">
...
<script>
import {getCourseList,getAllSubject} from '@/api/course'
export default {
  data() {
    return {
      current:1,
  ...
  methods: {
    //没有查询的默认数据
    initCourseFirst(){
      getCourseList(1,8,this.searchObj).then(response => {
    ...
    //分页切换方法
    gotoPage(current){
      getCourseList(current,8,this.searchObj).then(response => {
        this.data = response.data.data;
      })
    },
